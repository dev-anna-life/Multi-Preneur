...
document.addEventListener("click", (e) => {
  ...
  if (
    e.target.closest("#services-toggle") ||
    e.target.id === "services-toggle"
  ) {
    const servicesDropdown = document.getElementById("services-dropdown");
    const chevronIcon = document.getElementById("chevron-icon");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initUnlockPotentialHoverEffects() {
    const unlockCards = document.querySelectorAll(
      ".bg-\\[\\#29437f\\].rounded-xl",
    );
    ...
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        ...
        if (
          ...
          entry.target.parentElement.classList.contains("grid")
        ) {
          const gridItems =
            entry.target.parentElement.querySelectorAll(".opacity-0");
...
function displayUi(data) {
  ...
  data?.properties.forEach((property) => {
    ...
    div.className =
      "translate-y-8 transition-all duration-1000 ease-out opacity-0 hover:-translate-y-4";
    div.innerHTML = `
